...
class Init():
  ...
  def load_book_info(self):
    global book_info
    with open('book_info.json') as file:
...
def return_book(book_num):
    ...
    for i in borrowed_book:
        if i[0] == book_num:
            if messagebox.askokcancel('還書',f'座號: {i[1]},圖書編號: {book_num},書名: {book_info[book_num]}'):
...
class Mode():
    ...
    def view(self):
        ...
        for i in delayed_list:
            if delayed_list[i] > time.time():
                ...
                printing.append(f'座號:{i},懲罰結束時間:{end_time.tm_year-1911}年{end_time.tm_mon}月{end_time.tm_mday}日')
    ...
    def delete(self):
        ...
        if messagebox.askokcancel('刪除懲罰者',f'座號:{stu_num},懲罰結束時間:{end_time.tm_year-1911}年{end_time.tm_mon}月{end_time.tm_mday}日,確認刪除?'):
